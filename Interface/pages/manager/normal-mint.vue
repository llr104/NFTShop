	...
	<view>
		...
		<view class="ptdz">
			<uni-forms :modelValue="data">
				...
				<uni-forms-item required name="price" label="NFT价格">
					<uni-easyinput type="number" v-model="data.price" placeholder="请输入NFT价格" />
				</uni-forms-item>
			</uni-forms>
				...
				uni.showLoading({
					...
					eth.getAccounts((error, result)=>{
						...
						if(!error && result.length != 0){
							...
							let _uri = this.data.uri;
							let _count = this.data.count;
							let _des = this.data.des;
							let _price = tokens.toTokenValue(this.data.price);
							let _group = -1;
							let _type = 1;
			
							nft.methods.mint(_to, _name, _uri, _des, _type, _count, _group, _price, 0).send({from: _from}, (error, result)=>{
